...
**/(function(){var c=this.createjs=this.createjs||{},c=c.PreloadJS=c.PreloadJS||{};c.version="0.3.1";c.buildDate="Thu, 25 Apr 2013 20:34:58 GMT"})();this.createjs=this.createjs||{};
...
null)){b=b.getPreloadHandlers();if(b.types!=null)for(var e=0,a=b.types.length;e<a;e++)this._typeCallbacks[b.types[e]]=b.callback;if(b.extensions!=null)for(e=0,a=b.extensions.length;e<a;e++)this._extensionCallbacks[b.extensions[e]]=b.callback}};a.setMaxConnections=function(b){this._maxConnections=b;!this._paused&&this._loadQueue.length>0&&this._loadNext()};a.loadFile=function(b,a,d){b==null?this._sendError({text:"PRELOAD_NO_FILE"}):(this._addItem(b,d),a!==false&&this.setPaused(false))};a.loadManifest=
function(b,a,d){var c=null;if(b instanceof Array){if(b.length==0){this._sendError({text:"PRELOAD_MANIFEST_EMPTY"});return}c=b}else{if(b==null){this._sendError({text:"PRELOAD_MANIFEST_NULL"});return}c=[b]}for(var b=0,j=c.length;b<j;b++)this._addItem(c[b],d);a!==false&&this.setPaused(false)};a.load=function(){this.setPaused(false)};a.getItem=function(b){return this._loadItemsById[b]||this._loadItemsBySrc[b]};a.getResult=function(b,a){var d=this._loadItemsById[b]||this._loadItemsBySrc[b];if(d==null)return null;
d=d.id;return a&&this._loadedRawResults[d]?this._loadedRawResults[d]:this._loadedResults[d]};a.setPaused=function(b){(this._paused=b)||this._loadNext()};a.close=function(){for(;this._currentLoads.length;)this._currentLoads.pop().cancel();this._scriptOrder.length=0;this._loadedScripts.length=0;this.loadStartWasDispatched=false};a._addItem=function(b,a){var d=this._createLoadItem(b);if(d!=null){var c=this._createLoader(d,a);c!=null&&(this._loadQueue.push(c),this._loadQueueBackup.push(c),this._numItems++,
...
createjs.LoadQueue.JSON&&a.callback!=null)a.type=createjs.LoadQueue.JSONP;if(a.type==createjs.LoadQueue.JSONP&&a.callback==null)throw Error("callback is required for loading JSONP requests.");if(a.tag==null)a.tag=this._createTag(a.type);if(a.id==null||a.id=="")a.id=a.src;if(b=this._typeCallbacks[a.type]||this._extensionCallbacks[a.ext]){b=b(a.src,a.type,a.id,a.data);if(b===false)return null;else if(b!==true){if(b.src!=null)a.src=b.src;if(b.id!=null)a.id=b.id;if(b.tag!=null&&b.tag.load instanceof Function)a.tag=
b.tag;if(b.completeHandler!=null)a.completeHandler=b.completeHandler}if(b.type)a.type=b.type;b=this._parseURI(a.src);if(b!=null&&b[5]!=null)a.ext=b[5].toLowerCase()}this._loadItemsById[a.id]=a;return this._loadItemsBySrc[a.src]=a};a._createLoader=function(b,a){var d=this.useXHR;switch(b.type){case createjs.LoadQueue.JSON:case createjs.LoadQueue.XML:case createjs.LoadQueue.TEXT:d=true;break;case createjs.LoadQueue.SOUND:case createjs.LoadQueue.JSONP:d=false}if(a==null)a=this._basePath;return d?new createjs.XHRLoader(b,
a):new createjs.TagLoader(b,a)};a._loadNext=function(){if(!this._paused){if(!this._loadStartWasDispatched)this._sendLoadStart(),this._loadStartWasDispatched=true;if(this._numItems==this._numItemsLoaded)this.loaded=true,this._sendComplete(),this.next&&this.next.load&&this.next.load();for(var b=0,a=this._loadQueue.length;b<a;b++){if(this._currentLoads.length>=this._maxConnections)break;var d=this._loadQueue[b];if(this.maintainScriptOrder&&d instanceof createjs.TagLoader&&d.getItem().type==createjs.LoadQueue.JAVASCRIPT){if(this._currentlyLoadingScript)continue;
this._currentlyLoadingScript=true}this._loadQueue.splice(b,1);this._loadItem(d);b--;a--}}};a._loadItem=function(b){b.addEventListener("progress",createjs.proxy(this._handleProgress,this));b.addEventListener("complete",createjs.proxy(this._handleFileComplete,this));b.addEventListener("error",createjs.proxy(this._handleFileError,this));this._currentLoads.push(b);this._sendFileStart(b.getItem());b.load()};a._handleFileError=function(b){var a=b.target;this._numItemsLoaded++;this._updateProgress();b={item:a.getItem()};
this._sendError(b);this.stopOnError||(this._removeLoadItem(a),this._loadNext())};a._handleFileComplete=function(b){var b=b.target,a=b.getItem();this._loadedResults[a.id]=b.getResult();b instanceof createjs.XHRLoader&&(this._loadedRawResults[a.id]=b.getResult(true));this._removeLoadItem(b);if(this.maintainScriptOrder&&a.type==createjs.LoadQueue.JAVASCRIPT)if(b instanceof createjs.TagLoader)this._currentlyLoadingScript=false;else{this._loadedScripts[this._scriptOrder.indexOf(a)]=a;this._checkScriptLoadOrder(b);
return}this._processFinishedLoad(a,b)};a._processFinishedLoad=function(a,d){this._numItemsLoaded++;this._updateProgress();this._sendFileComplete(a,d);this._loadNext()};a._checkScriptLoadOrder=function(){for(var a=this._loadedScripts.length,d=0;d<a;d++){var c=this._loadedScripts[d];if(c===null)break;c!==true&&(this._processFinishedLoad(c),this._loadedScripts[d]=true,d--,a--)}};a._removeLoadItem=function(a){for(var d=this._currentLoads.length,c=0;c<d;c++)if(this._currentLoads[c]==a){this._currentLoads.splice(c,
...
delete this._loadItemsBySrc[a.src]};a._createTag=function(a){var d=null;switch(a){case createjs.LoadQueue.IMAGE:return document.createElement("img");case createjs.LoadQueue.SOUND:return d=document.createElement("audio"),d.autoplay=false,d;case createjs.LoadQueue.JSONP:case createjs.LoadQueue.JAVASCRIPT:return d=document.createElement("script"),d.type="text/javascript",d;case createjs.LoadQueue.CSS:return d=this.useXHR?document.createElement("style"):document.createElement("link"),d.rel="stylesheet",
...
case "js":return createjs.LoadQueue.JAVASCRIPT;case "svg":return createjs.LoadQueue.SVG;default:return createjs.LoadQueue.TEXT}};a._sendFileProgress=function(a,d){if(this._isCanceled())this._cleanUp();else{var c={target:this,type:"fileprogress",progress:d,loaded:d,total:1,item:a};this.onFileProgress&&this.onFileProgress(c);this.dispatchEvent(c)}};a._sendFileComplete=function(a,d){if(!this._isCanceled()){var c={target:this,type:"fileload",loader:d,item:a,result:this._loadedResults[a.id],rawResult:this._loadedRawResults[a.id]};
a.completeHandler&&a.completeHandler(c);this.onFileLoad&&this.onFileLoad(c);this.dispatchEvent(c)}};a._sendFileStart=function(a){this.dispatchEvent({target:this,type:"filestart",item:a})};a.toString=function(){return"[PreloadJS LoadQueue]"};createjs.proxy=function(a,d){return function(){return a.apply(d,arguments)}};createjs.LoadQueue=c;if(!createjs.proxy)createjs.proxy=function(a,d){var c=Array.prototype.slice.call(arguments,2);return function(){return a.apply(d,Array.prototype.slice.call(arguments,
0).concat(c))}};var d=function(){};d.init=function(){var a=navigator.userAgent;d.isFirefox=a.indexOf("Firefox")>-1;d.isOpera=window.opera!=null;d.isChrome=a.indexOf("Chrome")>-1;d.isIOS=a.indexOf("iPod")>-1||a.indexOf("iPhone")>-1||a.indexOf("iPad")>-1};d.init();createjs.LoadQueue.BrowserDetect=d;if(!Array.prototype.indexOf)Array.prototype.indexOf=function(a){if(this==null)throw new TypeError;var d=Object(this),c=d.length>>>0;if(c===0)return-1;var f=0;arguments.length>1&&(f=Number(arguments[1]),f!=
f?f=0:f!=0&&f!=Infinity&&f!=-Infinity&&(f=(f>0||-1)*Math.floor(Math.abs(f))));if(f>=c)return-1;for(f=f>=0?f:Math.max(c-Math.abs(f),0);f<c;f++)if(f in d&&d[f]===a)return f;return-1}})();this.createjs=this.createjs||{};
